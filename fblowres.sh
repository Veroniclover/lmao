...
post_to_timeline(){
	...
	# dl files
	if [ -n "${thumbnail}" ]; then
		curl -sLf "${thumbnail}" -o thumb.jpg || exit_custom
	fi
	if [ -n "${vid_link}" ]; then
		curl -sLf "${vid_link}" -o vid.mp4 || exit_custom
	fi
	# upload now
	if [ -n "${vid_link}" ]; then
		...
		"${graph_url_main}/v16.0/me/videos")" || exit_custom
		id_post="$(printf '%s' "${id_post}" | sed -nE 's|.*id":"([^"]*)".*|\1|p')"
		[ -z "${id_post}" ] && exit_custom
	else
		id_post="$(curl -sLf -X POST \
			-F "access_token=${token}" \
			-F "source=@thumb.jpg" \
			-F "message=${capt_compose}" \
		"${graph_url_main}/v16.0/me/photos")" || exit_custom
		id_post="$(printf '%s' "${id_post}" | sed -nE 's|.*id":"([^"]*)".*|\1|p')"
		[ -z "${id_post}" ] && exit_custom
	fi
...
date_posted="$(sed -nE 's|.*creation_time":([^,]*).*|\1|p' <<< "${body}" | head -n 1 | grep '[^[:space:]]')" || exit_custom
...
group_name="$(printf '%s' "${body}" | sed -nE 's|.*"group":\{"name":"([^"]*)".*|\1|p' | grep '[^[:space:]]')" || exit_custom
